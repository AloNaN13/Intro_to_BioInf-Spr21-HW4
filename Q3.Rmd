...
```{r, eval=FALSE}
cox_diseases<-coxph(Surv(times, patient.vital_status)~admin.disease_code, data=clinkid)
cox_diseases
coxph(Surv(times, patient.vital_status)~patient.gender, data=clinkid)
```
- Concerning gender, we can see male has a better prognosis than females. the coef is negative but the value is rather small, so the improvement is not significant.
- According to the Cox PH test, the survival rate for kirp is worse than kich, and the survival rate for kirc is the worst. Therefore we can conclude that kirc (with the patient being female) has the worst prognosis.
...
```{r}
kid_surv<-survfit(Surv(times, patient.vital_status)~admin.disease_code, data=clinkid)
ggsurvplot(kid_surv, conf.int=TRUE, pval=TRUE)
summary(kid_surv, times=seq(0,365*5,365))
...
min_g <- 'some_str'
min_p <- 100
all_genes <-names(m)[6:(length(colnames(m))-1)]
for(gene in all_genes){
  print(gene)
  coxph_res<-coxph(Surv(times, patient.vital_status)~get(gene), data=m)
  curr_p <- summary(coxph_res)$coefficient[,5]
  if(curr_p < min_p){
    min_p<-curr_p
    min_g <-gene
  }
...
```{r}
#The gene which has the lowest p-value is HERC3 as we found out
g_median_val <- median(m$HERC3)
b_median <- filter(m,HERC3>=g_median_val)
s_median <- filter(m,HERC3<g_median_val)
s_median$median_status<-'smaller'
b_median$median_status<-'bigger or eq'
surv_m<-rbind(s_median, b_median)
ggsurvplot(surv_fit(Surv(times, patient.vital_status)~median_status,
                    data=surv_m))
```
